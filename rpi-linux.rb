...
package :raspberrypi_linux do
  ...
  VAR['RASPBERRYPI_LINUX_SHA'] ||= github_get_head('raspberrypi/linux', VAR['RASPBERRYPI_LINUX_BRANCH']) if VAR['RASPBERRYPI_LINUX_BRANCH']
  # else use the commit that was used to build the firmware
  VAR['RASPBERRYPI_LINUX_SHA'] ||= http_get("https://raw.githubusercontent.com/raspberrypi/firmware/#{VAR['RASPBERRYPI_FIRMWARE_SHA']}/extra/git_hash").to_s.strip
  ...
  target :build do
    ...
    File.open("#{dst}/git_hash", 'w') { |file| file.write(VAR['RASPBERRYPI_LINUX_SHA']) }
		mkdir_p(dst + "/extra")
